...
def verify_blocking_ground_truth(A, B, block_df, duplicates_df,
                                 objectify=False):
  ...
  merged_df = block_df.merge(duplicates_df, left_on=["ltable_id", "rtable_id"],
...
def blocking_for_citeseer_dblp(A, B):
  ...
  attributes = ['id', 'title', 'authors', 'journal', 'month', 'year',
                'publication_type']

  ob = em.OverlapBlocker()
  C1 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=2,
                       show_progress=True,
...
def block_baby_products(A, B):
  ...
  C = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=4,
                      show_progress=True, l_output_attrs=attributes,
...
def block_books(A, B):
  ...
  attributes = ['Title', 'Author', 'ISBN13', 'Publisher', 'Publication_Date',
                'Pages', 'Dimensions']
  # C = ob.block_tables(A, B, 'Title', 'Title', l_output_attrs=attributes,  r_output_attrs=attributes,
  #     overlap_size=1, show_progress=False)
  C = ob.block_tables(A, B, 'Title', 'Title', word_level=True, overlap_size=4,
...
def block_restaurants(A, B):
  ...
  #     overlap_size=1, show_progress=False)
  C = ob.block_tables(A, B, 'name', 'name', word_level=True, overlap_size=4,
                      show_progress=True,
                      l_output_attrs=attributes, r_output_attrs=attributes)
  return C
...
def block_dblp_scholar(A, B):
  ...
  C2 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=4,
                       show_progress=True, l_output_attrs=attributes,
                       r_output_attrs=attributes)
...
def block_rotten_imdb(A, B):
  ...
  # C1 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=3, show_progress=True, l_output_attrs=attributes, r_output_attrs=attributes)
  # =================>results in a candidate set of size 1.2M with 178 missing duplicates out of 5347
  C2 = ob.block_tables(A, B, 'Name', 'Name', word_level=True, overlap_size=2,
                       show_progress=True, l_output_attrs=attributes,
                       r_output_attrs=attributes)
  # =================>results in a candidate set of size 135K with 467 missing duplicates out of 5347
  return C2
...
def block_walmart_amazon_(A, B):
  ...
  attributes = ['brand', 'groupname', 'title', 'price', 'shortdescr',
                'longdescr', 'imageurl', 'modelno', 'shipweight', 'dimensions']
  C2 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=3,
                       l_output_attrs=attributes, r_output_attrs=attributes,
...
def block_walmart_amazon(A, B):
  ...
  r_attributes = ["title", "proddescrshort", "brand", "price", "dimensions",
                  "shipweight"]
  l_attributes = ["title", "shortdescr", "brand", "price", "dimensions",
                  "shipweight"]

  if not set(r_attributes).issubset(
      B.columns):  # fix in case A B are the same dataset
  ...
  C2 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=2,
                       l_output_attrs=l_attributes, r_output_attrs=r_attributes,
                       show_progress=True, allow_missing=True)
  # =================>results in a candidate set of size 278K with 84 missing duplicates out of 1154
  # blocking_utils.verify_blocking_ground_truth(dataset_name, C2)
  return C2
...
def block_amazon_googleproducts(A, B):
  ...
  C = ob.block_tables(A, B, "title", "title", word_level=True, overlap_size=1,
                      l_output_attrs=["title", "description", "manufacturer",
                                      "price"],
                      r_output_attrs=["title", "description", "manufacturer",
                                      "price"], show_progress=True,
                      allow_missing=False)
  return C


def block_amazon_google(A, B):
  ob = em.OverlapBlocker()
  C = ob.block_tables(A, B, "title", "title", word_level=True, overlap_size=1,
                      l_output_attrs=["title", "manufacturer", "price"],
                      r_output_attrs=["title", "manufacturer", "price"],
                      show_progress=True, allow_missing=True)
  return C
...
def block_beer(A, B):
  ...
  C = ob.block_tables(A, B, "Beer_Name", "Beer_Name", word_level=True, overlap_size=1,
                      l_output_attrs=['Beer_Name','Brew_Factory_Name','Style','ABV'],
                      r_output_attrs=['Beer_Name','Brew_Factory_Name','Style','ABV'],
                      show_progress=True, allow_missing=True)
  return C
...
def block_songs(A, B):
  ...
  # =================>results in a candidate set of size 400K with 6 missing duplicates out of 1300
  C = ob.block_tables(A, B, "title", "title", word_level=True, overlap_size=1,
                      l_output_attrs=["title", "release", "artist_name",
                                      "duration", "artist_familiarity",
                                      "artist_hotttnesss", "year"],
                      r_output_attrs=["title", "release", "artist_name",
                                      "duration", "artist_familiarity",
                                      "artist_hotttnesss", "year"],
...
def block_itunes_amazon(A, B):
  ob = em.OverlapBlocker()
  C = ob.block_tables(A, B, "Song_Name", "Song_Name", word_level=True,
                      overlap_size=1,
                      l_output_attrs=['Song_Name', 'Artist_Name', 'Album_Name',
...
blocking_functions_mapping[
  "amazon_googleproducts"] = block_amazon_googleproducts
